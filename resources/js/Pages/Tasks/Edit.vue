<template>
  <div class="min-h-screen bg-slate-50 flex flex-col p-6 font-sans relative overflow-hidden">
    ...
    <div class="pt-8 pb-6 relative z-10 animate-in flex items-center justify-between">
      ...
      <button @click="showDeleteModal = true" type="button" class="inline-flex items-center justify-center w-11 h-11 text-red-500 bg-white shadow-sm border border-red-100 hover:bg-red-50 hover:shadow-md rounded-full transition-all">
    ...
    <div class="mb-8 relative z-10 animate-in delay-100">
      <h1 class="text-[32px] font-black text-slate-800 tracking-tight mb-2">Edit Tugas</h1>
      <p class="text-[15px] text-slate-500 font-medium">Perbarui rincian tugas Anda</p>
    </div>
      ...
      <div class="pt-8 pb-10">
        ...
        >
          ...
          <span class="relative z-10 uppercase tracking-wider flex items-center" v-else>
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
            Simpan Perubahan
          </span>
        </button>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div class="bg-white rounded-3xl w-full max-w-[320px] overflow-hidden shadow-2xl">
        ...
        <div class="text-center px-6">
          <h3 class="text-[19px] font-black text-slate-800 mb-2">Hapus Tugas?</h3>
          <p class="text-[14px] text-slate-500 font-medium">Tugas ini akan dihapus secara permanen dan tidak bisa dikembalikan.</p>
        </div>
        <div class="p-6 flex gap-3 mt-2">
          <button type="button" @click="showDeleteModal = false" class="flex-1 py-3.5 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-2xl transition-colors text-[14px]">Batal</button>
          <button type="button" @click="deleteTask" class="flex-1 py-3.5 px-4 bg-red-500 hover:bg-red-600 text-white font-bold rounded-2xl transition-colors shadow-md shadow-red-200 text-[14px]">Ya, Hapus</button>
        </div>
      </div>
    </div>
  </div>
